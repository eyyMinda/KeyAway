...
function parseBody(body: unknown): { title?: string; slug?: string; description?: string; downloadLink?: string } {
  if (!body || typeof body !== "object") return {};
  const b = body as Record<string, unknown>;
  const out: { title?: string; slug?: string; description?: string; downloadLink?: string } = {};
  ...
  try {
    ...
    if (keys.length === 0) {
      return NextResponse.json(
        { error: "No valid fields to update (allowed: title, slug, description, downloadLink)" },
        { status: 400 }
      );
    }
