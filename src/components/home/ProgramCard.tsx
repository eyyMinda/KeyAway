...
export default function ProgramCard({ program, stats, badges, showStats = true }: ProgramCardProps) {
  ...
  const keyBadgeClass = hasKeys
    ? "bg-[#4c6b22] text-white border border-[#5c8529]"
    : "bg-[#2a2020] text-white border border-[#6d2626]";

  return (
    <div className="card-base group relative flex flex-col overflow-hidden rounded-sm">
      <Link href={`/program/${program.slug.current}`} title={program.title} className="relative overflow-hidden p-4">
        {program.image ? (
          <IdealImage
            image={program.image}
            alt={program.title}
          ...
          <div className="flex h-36 w-full items-center justify-center bg-[#213246] sm:h-40 lg:h-48">
            <div className="text-3xl text-neutral-100 sm:text-4xl">💻</div>
          </div>
        )}
        ...
        <div className="absolute top-2 sm:top-3 lg:top-4 left-2 sm:left-3 lg:left-4 flex flex-col gap-1.5 sm:gap-2">
          {badges?.mostViewed && (
            <div className="flex items-center space-x-1 rounded-sm bg-[#1a3a5c] px-2 py-0.5 text-xs font-semibold text-white sm:px-2.5 sm:py-1 sm:text-sm lg:px-3">
          ...
          {badges?.mostDownloaded && (
            <div className="flex items-center space-x-1 rounded-sm bg-[#1a3a2a] px-2 py-0.5 text-xs font-semibold text-white sm:px-2.5 sm:py-1 sm:text-sm lg:px-3">
        ...
        <div className="absolute top-2 right-2 sm:top-3 sm:right-3 lg:top-4 lg:right-4">
          <div
            className={`flex items-center space-x-1 rounded-sm px-2 py-1 text-xs font-semibold sm:px-2.5 sm:py-1.5 sm:text-sm lg:px-3 ${keyBadgeClass}`}>
      ...
      <div className="flex grow flex-col p-4 pt-0">
        ...
        {showStats && (stats?.viewCount !== undefined || stats?.downloadCount !== undefined) && (
          <div className="mt-auto mb-2 flex items-center justify-center gap-3 rounded-sm border border-[#2a475e] bg-[#16202d] px-2 py-1.5 text-xs text-neutral-100 sm:gap-4 sm:px-3 sm:py-2">
