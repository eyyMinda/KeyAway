  ...
  return (
    <div className="group bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 border-gray-200 hover:border-primary-400 animate-fade-in flex flex-col transform hover:-translate-y-2 relative before:absolute before:inset-0 before:bg-gradient-to-br before:from-primary-50/30 before:to-transparent before:opacity-0 before:group-hover:opacity-100 before:transition-opacity before:duration-300">
      ...
      <Link href={`/program/${program.slug.current}`} title={program.title} className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 border-2 border-transparent group-hover:border-primary-200/50 transition-colors duration-300 rounded-2xl" />
        ...
        {program.cdKeys?.length && (
          <div className="absolute top-4 right-4">
            <div className="bg-white/95 backdrop-blur-sm text-gray-800 px-3 py-1.5 rounded-full text-sm font-semibold flex items-center space-x-1 shadow-lg border border-gray-200">
              <FaKey className="w-3 h-3 text-primary-600" />
      ...
      {/* Content */}
      <div className="py-6 px-5 flex flex-col flex-grow bg-gradient-to-b from-white to-gray-50/50">
        <h2 className="text-xl font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-primary-700 transition-colors">
          {program.title}
        </h2>

        <p className="text-gray-600 text-sm mb-4 line-clamp-4 leading-relaxed">{program.description}</p>

        {/* Stats above CTA */}
        {showStats && (stats?.viewCount !== undefined || stats?.downloadCount !== undefined) && (
          <div className="flex items-center justify-center gap-4 text-xs text-gray-500 mb-4 bg-gray-50/80 rounded-lg py-2 px-3">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center w-full bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-200 transform hover:scale-[1.02] hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 mt-auto relative overflow-hidden">
          <span className="relative z-10">View Keys</span>
          <svg
            className="ml-2 w-4 h-4 transition-transform group-hover:translate-x-1 relative z-10"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24">
