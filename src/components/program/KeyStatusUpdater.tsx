...
export default function KeyStatusUpdater() {
  ...
  const updateKeyStatuses = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error("Failed to update key statuses:", result.message);
      }
    } catch (error) {
      console.error("Error updating key statuses:", error);
    } finally {
      setIsUpdating(false);
    }
  };
    ...
    <div className="bg-neutral-800 rounded-lg p-4 mb-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-300 text-sm">Manually update key statuses based on their validity dates</p>
          {lastUpdated && <p className="text-neutral-400 text-xs mt-1">Last updated: {lastUpdated.toLocaleString()}</p>}
        </div>
        <button
          onClick={updateKeyStatuses}
          disabled={isUpdating}
